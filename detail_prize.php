        ...
        <div class="row">
          <form class="custom-form" action="" method="POST" role="form" enctype="multipart/form-data">
            <div class="title-group mb-3 text-center">
              <h1 style="color:#787878; font-weight:bold; font-size:50px;" class="mb-0">獎品資料</h1>
              <br>
              <hr><br>
            </div>
            <div id="carouselExampleDark" class="carousel carousel-dark slide col-lg-12 col-md-12 col-18 text-center" data-bs-ride="carousel">
              <div class="carousel-indicators">
                ...
                <?php
                $result = mysqli_query($link, "SELECT * FROM `prize` WHERE `pid` = $pid");
                $row = mysqli_fetch_assoc($result);
                $result2 = mysqli_query($link, "SELECT iname FROM `prize_img` WHERE `pid` = $pid");
                $result3 = mysqli_query($link, "SELECT `maxpoint` FROM `activites` WHERE `aid` = " . $row['aid'] . "");
            ...
            <div class="container">
              <div class="row">
                <?php
                echo "<h2 style='font-weight:bold; color:#787878;' class='new_tit'>獎品名稱</h2>";
                echo "<h3>" . $row['pname'] . "</h3><br><br>";
                echo "<h2 style='font-weight:bold; color:#787878;' class='new_tit'>獎品點數</h2>";
                echo "<h3>" . $row['point'] . "</h3><br><br>";
                echo "<br>";
                if (isset($_SESSION['id'])) {
                ?>
                  <br>
                  ...
                  <table>
                    <tr>
                      <div class="col-lg-6 col-6">
                        <?php echo "<a href='update_prize.php?pid=" . $row['pid'] . "&aid=" . $row['aid'] . "&maxpoint=" . $row3['maxpoint'] . "' style='background-color:#FFA600' class='custom-btn btn'><input type='button' name='pupdata' value='　　　修改　　　' style='background-color:transparent;  border:none; color:white;' ></a>"; ?>
                      </div>
                    </tr>
                    <tr>
                      <div class="col-lg-6 col-6">
                        <?php echo "<a href='prize.php?aid=" . $row['aid'] . "&maxpoint=" . $row3['maxpoint'] . "' style='background-color:#FFA600' class='custom-btn btn'><input type='button' value='　　　返回　　　' style='background-color:transparent;  border:none; color:white;' ></a>"; ?>
                      </div>
                    </tr>
                  </table>
                <?php  } ?>
  ...
  <script>
    $(function() {
      ...
      function createqrcode() {
        var input_text = "http://127.0.0.1/qrfun0828/qrcode_prize.php?pid=" + <?php echo $pid ?>; //qrcode網址
